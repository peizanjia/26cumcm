...
源码与实验交付已于2026-09-13推送到`origin/main`，功能提交`d4e8df4`。101项测试、300任务90805条动作审计、桌面与手机真实浏览器QA均通过。
...
演练与正式测试使用 [question4/main.py](../main.py)，默认已加载本目录冻结031完整参数。支持与第三问相同的常驻等待、手动开始一局执行一局、正式模式标记和每局历史保存；结束后输出检测到的目标数，并另存纯数字文件。命令与接口限制见 [运行说明](../OFFICIAL_RUN.md)。
