...
class Planner(BasePlanner):
    ...
    def _forecast(self, q, known_values, selected_channels, clearing=None):
        ...
        scan_order = list(selected_channels)
        row = getattr(self, '_evaluated_row', None)
        if row is not None and row['kind'] == 'measure' and scan_order:
            values = self._unknown_values(q, row['required']) + list(known_values)
            selected = [r for r in values if r['channel'] in selected_channels]
            primary = row['owner']
            if primary is None:
                primary = max(selected, key=lambda r: (r.get('required', False), r['net_saving_s']))['channel']
            scan_order = [primary] + [c for c in scan_order if c != primary]
        receiver = scan_order[-1] if scan_order else self.channel
        scans = 0.
        for i in order:
            if i < len(known):
                continue
            needed = self.unknown_map.required_channels(nodes[i], channels)
            if np.array_equal(nodes[i], q):
                # A partial scan may not remove this whole shared future site.
                # Its selected channels have nevertheless already been priced.
                needed = [c for c in needed if c not in selected_channels]
    ...
    def _evaluate_once(self, row):
        previous = getattr(self, '_evaluated_row', None)
        self._evaluated_row = row
        try:
            return super()._evaluate(row)
        finally:
            self._evaluated_row = previous

    def _evaluate(self, row):
        ordinary = self._evaluate_once(row)
        ...
        if len(after) >= len(plan):
            return ordinary
        bundled = self._evaluate_once(dict(row, required=required))
        if bundled is None:
            return ordinary
