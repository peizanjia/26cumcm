...
def resolve_configs(specification):
    ...
    for name, spec in specification.items():
        policy = spec.get("policy", "old_joint" if name == "old_joint" else "dynamic")
        if policy == "old_joint":
...
def code_hashes():
    paths = [*BASE.glob("*.py"), BASE.parent / "full_mission" / "planner.py",
...
def make_replays(validation, output, workers, seeds=None):
    ...
    if seeds:
        ...
        current = {r["seed"]: r for r in rows if r["strategy"] == "dynamic_joint" and r["completed"]}
...
def main():
    ...
    if args.variants:
        ...
        specs = {"old_joint": {"policy": "old_joint"}, "dynamic_joint": {"policy": "dynamic"}}
    ...
    if args.freeze_from:
        ...
        configs = {"old_joint": resolve_configs({"old_joint": {"policy": "old_joint"}})["old_joint"],
                   "dynamic_joint": {**data["configs"][args.freeze_from], "label": LABELS["dynamic_joint"]}}
