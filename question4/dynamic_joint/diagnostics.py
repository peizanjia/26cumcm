...
SHORT = {"old_joint": "旧联合", "dynamic": "仅动态未知域", "dynamic_joint": "动态联合"}
PALETTE = {"old_joint": "#7294aa", "dynamic": "#bd995b", "dynamic_joint": "#408d79"}
...
def overview(data, output, smoke=False):
    ...
    if paired:
        ...
        ax.margins(y=.23)
    ax.set_title("动态联合 − 旧联合：同场景配对差", loc="left", pad=14)
    ax.set_xlabel("共同场景按差值排序；负数表示动态联合更快")
    ...
    title = "绘图测试：开发样本，不能作为新验证成绩" if smoke else f"第四问动态联合 · {len(data.get('seeds', []))} 个共同场景／每策略 {source_count} 个源"
...
def paired_rows(rows):
    a = {r["seed"]: r for r in rows if r["strategy"] == "dynamic_joint" and r.get("completed")}
...
def select_cases(data):
    ...
    if pairs:
        ...
        tag(max(pairs, key=lambda p: p[1]["per_source_s"])[0], "动态联合最慢")
    return selected
...
def case_plot(seed, histories, tags, output, smoke=False):
    names = [n for n in ("dynamic_joint", "old_joint") if n in histories]
    if len(names) < 2:
    ...
    delta = histories["dynamic_joint"]["summary"]["per_source_s"]-histories["old_joint"]["summary"]["per_source_s"]
...
def smoke_data():
    files = [BASE/"outputs"/"current_smoke"/"dynamic_joint"/"20285000.json.gz",
...
def main():
    ...
    for seed, tags in selected.items():
        histories = {r["strategy"]: load_history(r, args.input.parent) for r in data["rows"]
                     if r["seed"] == seed and r["strategy"] in ("dynamic_joint", "old_joint")}
        if len(histories) < 2:
            continue
        case = case_plot(seed, histories, tags, output, smoke=args.smoke)
        if args.unknown_overlay:
            case["unknown_overlay"] = unknown_overlay(seed, histories["dynamic_joint"], output)
