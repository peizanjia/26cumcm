"""Integration check against a fresh, private-port local HTTP server."""
import json
...
def main():
    out=Path('question3/global_policy/outputs/http_smoke');out.mkdir(parents=True,exist_ok=True)
    ...
    try:
        ...
        planner=Planner(HTTPClient('local',f'http://127.0.0.1:{port}'))
        result=planner.run()
        for _ in range(100):
            try:
                truth=json.loads((out/'evaluator/truth.json').read_text(encoding='utf8'));break
        ...
        result.update(true_total=len(truth['sources']),true_cleared=sum(s['cleared'] for s in truth['sources']),
                      evaluation='local_synthetic_http')
