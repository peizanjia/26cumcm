...
“以区域直径为直径的圆能否覆盖区域”的一般答案是不能保证。边长为 D 的等边三角形，其区域直径为 D，但覆盖三个顶点至少需要半径 D/√3，大于 D/2。以某一最远顶点对的中点为圆心、D/2 为半径也因此不是通用覆盖方案。需要覆盖时，应另求最小包围圆，不能直接把区域直径除以二当成覆盖半径。

`enclosing_circle.py` 现已实现最小包围圆和覆盖判定。新增的 [80,000 组测向交集实验](experiments/diameter_circle/REPORT.md) 包含随机反例、只需三个合法测点的解析反例、不同测点数及实际多边形边数的统计。运行方式：
...
这里的 `--trials` 是每种配置/噪声组合使用的源种子数；默认 2 种配置 × 2 种噪声 × 8 个嵌套测点数量，共 80,000 次交集评估。保存了所有原始统计行、参数和反例坐标，可重复运行。输出默认放在 `question1/output/circle_study/`，本次结果另保存在上面的实验目录。
